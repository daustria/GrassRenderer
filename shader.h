...
// This is just straight from learnopengl.com (the other constructor is from a file called shader_t.h in the tutorial for tessellation).
class Shader {
public:
    unsigned int ID;
    ...
    {
        GLuint retval = 0;
        glGetUniformuiv(ID, glGetUniformLocation(ID, name.c_str()), &retval);
        return retval;
    }
    // ------------------------------------------------------------------------
